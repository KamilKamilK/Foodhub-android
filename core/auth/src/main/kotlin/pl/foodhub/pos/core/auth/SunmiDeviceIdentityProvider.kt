 ...
 * device identifier on Sunmi terminals, falling back to the same ANDROID_ID logic as
 ...
 *
 * This reads the standard Android serial API, not Sunmi's own inner-device SDK — we don't
 * have a Sunmi developer account or its proprietary `.aar` yet. Swap [deviceId] for the
 * real Sunmi SDK call once that dependency is in hand.
 */
class SunmiDeviceIdentityProvider
    @Inject
    constructor(
